var factoryAPI = 'http://localhost:3000/productFactorySold';
let currentPge = 1;
let perPage = 5;
let totalPage = 0;
let perUser = []
...
function deleteStudent(id) {
    alert('Xóa sản phẩm đã nhập Đại lý thành công');
    var options = {
        method: 'DELETE',
        headers: {
            'Content-Type': 'application/json'
...
function addNew(){
    ...
    if (checkErrorElement) {
        ...
        let nameFactory = document.getElementById('nameFactory').value;
        let codeProduct = document.getElementById('codeProduct').value;
        let yearday = document.getElementById('yearday').value;
        ...
        alert('Thêm sản phẩm đã nhập Đại lý thành công');
...
function renderStudent(factory){
    let student = `<tr>
        <th>ID</th>
        <th>Mã CSXS</th>
        <th class="restable">Tên CSXS</th>
        <th>Mã sản phẩm</th>
        <th class="restable">Năm sản xuất</th>
        <th>Ngày nhập Đại lý</th>
    ...
    let listStudent = factory.map((value, index)=>{
        student += `<tr>
            ...
            <td class="restable">${value.nameFactory}</td>
            <td>${value.codeProduct}</td>
            <td class="restable">${value.yearday}</td>
            <td>${value.dateday}</td>
            <td>
...
function editStudent(id) {
    ...
    fetch(factoryAPI + '/' + id)
        ...
        .then((data) => {
            ...
            document.getElementById("nameFactory").value = data.nameFactory;
            document.getElementById("codeProduct").value = data.codeProduct;
            document.getElementById("yearday").value = data.yearday;
...
function changeStudent() {
    ...
    let formData = {
        ...
        nameFactory: document.getElementById('nameFactory').value,
        codeProduct: document.getElementById('codeProduct').value,
        yearday: document.getElementById('yearday').value,
        dateday: document.getElementById('dateday').value
    };
    updateFactory(formData, id);
    alert('Thay đổi sản phẩm đã nhập đại lý thành công');
}
